...
const savedFontSize = localStorage.getItem("font_size") || "2";
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.shiftKey && e.key.toLowerCase() === "c") {
    e.preventDefault();
    if (addClipBtn) addClipBtn.click();
  }
});
...
function renderSearchResults(results) {
  ...
  results.forEach(({ item, path }) => {
    ...
    let iconHtml;
    if (item.type === "folder") {
      iconHtml = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>`;
    } else {
    ...
    resultItem.innerHTML = `${iconHtml}${contentHtml}`;
...
function renderColumn(items, depth, isActive) {
  ...
  items.forEach((item, index) => {
    ...
    if (item.type === "folder") {
      ...
      iconHtml = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon text-gray-500 dark:text-white/60 mt-[2px]"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>`;
    }
    ...
    const arrow =
      item.type === "folder"
        ...
        : "";
    row.innerHTML = `${iconHtml}<span class="text-[14px] flex-1 break-words text-gray-900 dark:text-white">${item.name}</span>${optionsBtn}${arrow}`;
